...
class MKV:
        ...
        for seek in seek_head:
            ...
            if element_name == "Info":
                ...
                self.info = Info.fromelement(
                    ebml.parse_element(stream, specs, True, ignore_element_names=["Void", "CRC-32"])
                )
                ...
                self.video_tracks.extend(
                    [VideoTrack.fromelement(t) for t in tracks if t["TrackType"].data == VIDEO_TRACK]
                )
                self.audio_tracks.extend(
                    [AudioTrack.fromelement(t) for t in tracks if t["TrackType"].data == AUDIO_TRACK]
                )
                self.subtitle_tracks.extend(
                    [SubtitleTrack.fromelement(t) for t in tracks if t["TrackType"].data == SUBTITLE_TRACK]
                )
            elif element_name == "Chapters":
                logger.info("Processing element %s from SeekHead at position %d", element_name, element_position)
                stream.seek(element_position)
                self.chapters.extend(
                    [
                        Chapter.fromelement(c)
                        for c in ebml.parse_element(stream, specs, True, ignore_element_names=["Void", "CRC-32"])[0]
                        if c.name == "ChapterAtom"
                    ]
                )
                ...
                self.tags.extend(
                    [
                        Tag.fromelement(t)
                        for t in ebml.parse_element(stream, specs, True, ignore_element_names=["Void", "CRC-32"])
                    ]
                )
    ...
    def to_dict(self):
        return {
            ...
            "video_tracks": [t.__dict__ for t in self.video_tracks],
            "audio_tracks": [t.__dict__ for t in self.audio_tracks],
            "subtitle_tracks": [t.__dict__ for t in self.subtitle_tracks],
...
class Info:
    ...
    def __init__(
        self, title=None, duration=None, date_utc=None, timecode_scale=None, muxing_app=None, writing_app=None
    ):
        self.title = title
    ...
    def __repr__(self):
        return "<%s [title=%r, duration=%s, date=%s]>" % (
            self.__class__.__name__,
            self.title,
            self.duration,
            self.date_utc,
...
class Track:
    ...
    def __init__(
        ...
        default=None,
        forced=None,
        lacing=None,  # @ReservedAssignment
        codec_id=None,
        codec_name=None,
    ):
        self.type = type
...
class VideoTrack(Track):
    ...
    def __repr__(self):
        return "<%s [%d, %dx%d, %s, name=%r, language=%s]>" % (
            self.__class__.__name__,
            self.number,
            self.width,
            self.height,
...
class AudioTrack(Track):
    ...
    def __init__(
        self, sampling_frequency=None, channels=None, output_sampling_frequency=None, bit_depth=None, **kwargs
    ):
    ...
    def __repr__(self):
        return "<%s [%d, %d channel(s), %.0fHz, %s, name=%r, language=%s]>" % (
            self.__class__.__name__,
            self.number,
            self.channels,
...
class SimpleTag:
    ...
    def __repr__(self):
        return "<%s [%s, language=%s, default=%s, string=%s]>" % (
            self.__class__.__name__,
            self.name,
            self.language,
            self.default,
...
class Chapter:
    ...
    def fromelement(cls, element):
        ...
        if len(chapterdisplays) > 1:
            logger.warning(
                "More than 1 (%d) ChapterDisplay element in the ChapterAtom, using the first one", len(chapterdisplays)
            )
        if chapterdisplays:
